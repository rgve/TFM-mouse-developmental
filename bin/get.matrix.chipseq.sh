...
for i in `seq 1 $n`; do
	...
	~bborsari/bwtool/bwtool summary -header -with-sum -keep-bed $bed_coordinates $file "$outFolder"/"$(basename $file .bw)".all.genes.summary.tsv
	if [[ "$i" == 1 ]]
	then 
		echo "$outFolder/"$(basename $file .bw)".all.genes.summary.tsv" > "$target".path.bwtool.summary.txt
	else
		echo "$outFolder/"$(basename $file .bw)".all.genes.summary.tsv" >> "$target".path.bwtool.summary.txt
	fi
done
...
~bborsari/bin/selectColumns.sh name:"$signal" "$file1" | tail -n+2 |awk 'BEGIN{FS=OFS="\t"}{split($1, a, "_"); print a[1], $2}' | sort -k1,1 -k2,2gr | sort -u -k1,1 > "$outFile"
...
tail -n+2 "$target".path.bwtool.summary.txt | while read file; do
	...
	~bborsari/bin/selectColumns.sh name:"$signal" "$file" | tail -n+2 | awk 'BEGIN{FS=OFS="\t"}{split($1, a, "_"); print a[1], $2}' | sort -k1,1 -k2,2gr | sort -u -k1,1 > tmp
...
~bborsari/bin/add_header.sh "$outFile" > tmp
mv tmp "$outFile"
