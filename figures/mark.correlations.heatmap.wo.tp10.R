setwd("/no_backup/rg/ragarcia/MBrain/H3K4me2")
library(ggplot2)

expression <- read.table('../expression/MBrain/expression.r10.tsv', sep="\t" , header=T, row.names = NULL)
chrom <- read.table('H3K4me2.matrix.after.QN.merged.upreg25.tsv', sep="\t")
...
colnames(zchrom) <- c('11.5',	'12.5',	'13.5',	'14.5',	'15.5',	'16.5',	'PN')
zchrom <- zchrom[rownames(expression2), ]

pheatmap(zchrom, cluster_cols = FALSE, cluster_rows=F, main = 'H3K4me2 Upreg >25 TPM',clustering_method = 'ward.D2', show_colnames = T, show_rownames = F, annotation_row = metaex )
